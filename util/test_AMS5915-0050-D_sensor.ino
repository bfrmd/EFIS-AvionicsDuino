...
#include "Wire.h"
float diffPressure, previousFilteredDiffPressure;
...
if (statusCapteur < 0) 
  {
    Serial.print("An error occured when communicating with the AMS5915-0050-D sensor. Check wiring, pull up resistors, I2C address, and I2C bus number (wire, wire1, wire2...)");
    while(1){}
  }
}
...
void loop() 
{
  delay(100);
  readSensorUnits();
  diffPressure = IIRFilteredValue (previousFilteredDiffPressure, diffPressure , 0.05); // IIR filter is applied 
...
{ 
  ...
  if (numBytes == sizeof(bufferI2C))                                                                  // put the data in buffer
  {
    ...
    *temperatureCounts = (((uint16_t) (bufferI2C[2])) <<3) + (((uint16_t) bufferI2C[3]&0xE0)>>5);
    ...
    Serial.print(filteredPressureCount,0);                      // when diffPressure is zero, should be as close as possible to minOutputCount (1638) after a few seconds
    statusCapteur = 1;
  } 
  else 
  {
